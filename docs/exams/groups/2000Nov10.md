...
??? info "Solution 3(a)"
    ...
    **Theorem.** (Class Equation) Suppose $G$ is a group. Then

    $$|G| = |Z| + \sum_{g\in T} |G : C(g)|,$$

    where $Z$ denotes the center of $G$, $C(g):= \{x \in G: xg = gx\}$ is the centralizer of $g$, and $T$ contains one element from each non trivial conjugacy class of $G$.
