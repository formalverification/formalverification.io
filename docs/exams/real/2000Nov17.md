...
7. Suppose that $f$ is real valued and integrable with respect to Lebesgue measure $m$ on $\mathbb R$ and that there are real numbers $a<b$ such that

    $$a \cdot m(U) \leq \int_U f \ dm \leq b \cdot m(U),$$

    for all open sets $U$ in $\mathbb R$.  Prove that $a \leq f(x) \leq b$ a.e.
