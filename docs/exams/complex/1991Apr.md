...
4. Let $\{P_n(z)\}$ be a sequence of polynomials in the variable $z$ and let $f(z)$ be a function defined for $\Omega = \{z : 1 < |z| < 2\}$. Suppose that

    $$\lim_{n\to \infty} \left[\sup_{z\in \Omega} |f(z)- P_n(z)|\right] = 0.$$

    Prove that there is a function $F(z)$, analytic for $|z| < 1$, that satisfies $F(z) = f(z)$ for all $z \in \Omega$.
...
6. Suppose that $u(z)$ is a continuous function defined on the open subset $\Omega \subset \mathbb C$. Assume that for each disk $D(a,r) = \{z \in \mathbb C : |z - a| \leq r\}$ contained in $\Omega$ we have

    $$u(a) = \frac{1}{\pi r^2}\iint_{D(a,r)}u(z)\ dx\ dy.$$

    Prove that $u$ is a harmonic function on $\Omega$.
